...
const CategoryAddModal: React.FC<CategoryAddModalProps> = ({
  ...
  const [isLoading, setIsLoading] = useState({ formSubmit: false });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setTouched({ label: true, emoji: true });
    ...
    setIsLoading({ formSubmit: true });

    try {
      const res = await api.post("/product/add/category", payload);
      if (res.status === 201) {
  ...
  return (
    <Modal open={open} onClose={handleClose} size="sm" title="Add New Category">
      <form onSubmit={handleSubmit}>
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <input
                ...
                placeholder="Enter category label (e.g., Beverages)"
          ...
          <div className="space-y-2">
            ...
            {errors.emoji && touched.emoji && (
              <p className="text-red-600 text-sm flex items-center gap-1">
                <span>⚠️</span> {errors.emoji.details}
              </p>
            )}
          </div>
