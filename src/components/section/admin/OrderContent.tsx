...
import OrderView from "@/components/modal/OrderView";
...
const OrdersContent = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const req = await api.get(`/order/get/all?${params.toString()}`);
                ...
                {orders.length > 0 ? (
                  orders.map((order) => (
                    <tr key={order.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                        ৳
                        {(order.meal_budget * order.total_meal).toLocaleString(
                          "en-BD"
                        )}
                      </td>
